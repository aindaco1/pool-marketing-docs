...
- You may change or cancel a pledge through its secure management link while the campaign is still open, even if it has already reached its goal. These changes and cancellations close when the campaign deadline passes.
...
- A failed or expired payment method can prevent a pledge from being completed. We may contact you to update it, but we do not promise that every failed payment can be recovered. If payment failed for a funded campaign whose deadline has passed, saving a new payment method can trigger an immediate retry of the saved pledge amount.

## 3. Prices, payment, tax, and optional charges

Prices are shown in U.S. dollars. The server verifies prices, selected variants, shipping, tax, tips, add-ons, and campaign totals when you make or change a pledge. New add-on selections, including a different variant, use current catalog prices. An existing add-on keeps its saved unit price when the product and variant are unchanged, even when you change the quantity. Campaign charges use the saved pledge amounts. Browser-displayed totals may remain estimates until enough destination information is available.
...
- Physical rewards and products ship to the address saved with the pledge. You are responsible for checking it and using the pledge-management link to submit permitted changes while the campaign is open. After the deadline, contact [support@example.com](mailto:support@example.com) about an address problem; a change may not be possible once fulfillment is underway.
...
We send pledge confirmations, payment notices, and fulfillment messages to administer your pledge. Campaign diary, milestone, and announcement emails include a campaign unsubscribe option. Using it does not cancel your pledge or opt you out of essential pledge and payment notices. Optional launch and abandoned-checkout reminders require the consent described at signup and include an unsubscribe method.

A delayed or undeliverable confirmation email does not undo a saved pledge. If you are unsure whether a pledge was saved, contact [support@example.com](mailto:support@example.com) before submitting another one.

Each secure pledge-management link authorizes access to a particular pledge. Management, supporter-community, preview, and administration links must not be shared with unauthorized people. Access can be revoked when a link expires, is misused, or no longer points to an active record.

## 9. Privacy and data

The Pool collects information needed to operate the service, including contact details, pledge and order data, campaign interactions, referral information, email delivery preferences, and shipping information for physical items. Stripe handles full payment-card details. The public site is hosted on GitHub Pages and uses Cloudflare infrastructure; Cloudflare also runs the Worker that manages pledges and administration. Resend delivers email, and shipping, tax, and abuse-prevention providers receive limited data when their service is needed.
