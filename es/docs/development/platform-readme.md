...
Hito de lanzamiento actual: **v1.1.2**. Esta versión refuerza la verificación del mapa del sitio implementado, agrega páginas de productos con recompensas destacadas cerradas por fallas para futuras incorporaciones a Google Shopping, publica políticas claras de envío y no devoluciones y actualiza la copia pública de Acerca de y Términos en inglés y español.
...
El contenedor de desarrollo Worker ahora se ejecuta en el nodo 24 para coincidir con las acciones GitHub. Wrangler 4.110 también se ejecuta contra el Worker `compatibility_date = "2026-05-03"` compartido, por lo que el comportamiento local de Miniflare/Workers permanece alineado con la semántica del tiempo de ejecución implementado.
...
- Los complementos a nivel de paquete ahora se pueden seleccionar en el sidecar del carrito, anclarlos a una campaña en carritos de múltiples campañas y editarlos más tarde desde Administrar aporte.
...
- Los informes de aporte y cumplimiento ahora separan el valor del aporte de campaña del valor adicional de la plataforma para facilitar las operaciones.
...
- `seo` para botones de identidad SEO limitados como `x_handle`, `same_as`, `default_social_image_alt`, `og_locale_overrides` y si el centro de la comunidad pública debe permanecer indexable
...
[`_config.local.yml`](https://github.com/your-org/your-project/blob/main/_config.local.yml) ahora es intencionalmente delgado: solo debe contener anulaciones locales verdaderas como URL de host local, `show_test_campaigns` y claves en blanco públicas Turnstile solo locales, no una segunda copia de la configuración base.
...
Los creadores pueden utilizar la [Lista de verificación para creadores de campañas](https://github.com/your-org/your-project/blob/main/creator-campaign-checklist.md) pública] para la preparación del lanzamiento. Cubre cambios recientes de cara al creador, incluidos complementos de campaña, inserciones alojadas, enlaces de referencia/QR, preparación de Blast para patrocinadores, planificación de vista previa de enlaces compartidos/sociales, carga de medios en el panel, expectativas de impuestos/envío, decisiones de envío gratuito y tasas de reserva, destinatarios de informes y transferencia de cumplimiento; la ruta española vive en `/es/creator-campaign-checklist/`.
...
El inicio de sesión del correo electrónico del administrador también puede usar Cloudflare Turnstile. Configure la clave del widget público en `_config.yml` como `admin.turnstile_site_key` y almacene el `TURNSTILE_SECRET_KEY` correspondiente como un secreto Worker. La automatización local/de prueba puede usar `ADMIN_TURNSTILE_BYPASS=true`, pero esa omisión debe permanecer fuera del Workers implementado.
...
- Las rutas de escritura de nivel limitado ahora solicitan al coordinador disponibilidad según la reserva en lugar de reconstruir la verdad a partir de las claves de reserva KV.
...
Estos son escenarios de planificación aproximados, no garantías. Asumen los TTL de caché del navegador predeterminados de 5 minutos, en su mayoría un comportamiento normal del usuario y los precios/límites actualmente publicados de Workers y KV.

|Escenario|Actividad diaria dura|Perspectivas del plan|
|----------|----------------------|--------------|
|Pequeño lanzamiento colectivo|~1500 visitas a la página de la campaña, ~75 visitas de administradores/colaboradores, ~20 inicios de pago, ~10 aportes completadas|Lo gratuito debería seguir siendo viable. Esta es la forma operativa que The Pool está diseñado para manejar de forma económica.|
...
A partir del 18 de abril de 2026, Cloudflare documenta el plan gratuito Workers en solicitudes de `100,000` por día. El plan pago Workers comienza en `$5/month` e incluye solicitudes `10 million` por mes más CPU `30 million` ms por mes antes del precio excedente. Workers KV Gratis incluye lecturas `100,000`/día más solo escrituras `1,000`/día y solicitudes de lista `1,000`/día, mientras que Workers KV en el plan pago incluye lecturas `10 million`/mes y escrituras `1 million`/mes antes de excedentes:
...
- `pledge-report.sh` es una exportación de libro mayor/historial, por lo que los aportes modificados aparecen como deltas y los cambios mixtos ahora mantienen el contexto de actualización de sugerencias en la columna `items`.
...
- `CLOUDFLARE_DNS_API_TOKEN`, `CLOUDFLARE_ZONE_ID` y `CLOUDFLARE_ZONE` opcionales para el flujo de trabajo de publicación de evidencia del proveedor. El token DNS debe ser de solo lectura con Zona/DNS/Lectura para la zona de producción.
