...
Hito de lanzamiento actual: **v1.0.3**. El conjunto de funciones v1.0 y el pase de refuerzo de lanzamiento están completos; v1.0.3 agrega correcciones de rendimiento público guiadas por PageSpeed, variantes WebP responsivas con un tramo móvil `640w`, videos hero de YouTube diferidos, reprocesamiento manual completo de medios y actualizaciones de optimización de medios del panel que conservan los archivos fuente.
...
- **Canal de optimización de medios del panel**: los medios cargados en el panel conservan el origen en el Worker, luego las herramientas del repositorio pueden comprimir imágenes sin pérdidas, generar variantes WebP responsivas para el navegador incluido un tramo móvil `640w` y generar derivados WebM de alta calidad para los videos cargados.
- **Incrustaciones de video remoto diferidas**: los videos hero de YouTube de las campañas se muestran primero como una fachada local con póster/botón de reproducción y cargan el iframe remoto solo después de la intención de reproducción del colaborador.
...
- [PERFORMANCE.md](/es/docs/operations/performance/) — Modelo de rendimiento de la plataforma, minificación de activos generados, compresión de Cloudflare, carga en tiempo de ejecución, almacenamiento en caché, medios, videos hero de YouTube diferidos y captación previa pública segura
...
Los medios cargados en el panel conservan el origen cuando ingresan al repositorio. El flujo de trabajo independiente **Optimizar medios del panel** se ejecuta en `main` para los cambios `assets/images/**`, `assets/videos/**`, `_campaigns/**` y `_config.yml`; comprime imágenes cuando hay una salida más pequeña disponible, genera variantes WebP responsivas para plantillas públicas de imagen en `320w`, `480w`, `640w`, `960w` y `1600w`, genera derivados WebM para videos subidos, reescribe referencias de video literales después de que existen derivados y confirma esos cambios de optimización con el bot GitHub Actions. Use la opción manual `scope=all` del flujo de trabajo cuando los medios existentes de una campaña necesiten reprocesamiento completo.
